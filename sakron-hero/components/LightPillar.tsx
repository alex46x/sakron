...
const LightPillar: React.FC<LightPillarProps> = ({
  ...
  useEffect(() => {
    ...
    const qualitySettings = {
      ...
      medium: { iterations: 24, waveIterations: 1, pixelRatio: 0.75, precision: 'mediump', stepMultiplier: 1.3 },
      high: {
        iterations: 48,
        waveIterations: 2,
        pixelRatio: 1,
    ...
    // ponytail: render at half-res, upscale via CSS — for a glowing gradient
    // this drops fragment cost to ~25% with no visible quality loss.
    renderer.domElement.style.width = '100%';
    renderer.domElement.style.height = '100%';
    renderer.setSize(width * 0.5, height * 0.5, false);
    ...
    const material = new THREE.ShaderMaterial({
      ...
      uniforms: {
        uTime: { value: 0 },
        uResolution: { value: new THREE.Vector2(width * 0.5, height * 0.5) },
    ...
    // ponytail: cap everyone at 30fps — a glowing gradient at 30 looks
    // identical to 60, but halves the raymarch cost per second.
    const targetFPS = 30;
    ...
    const handleResize = () => {
      ...
      resizeTimeout = window.setTimeout(() => {
        ...
        // ponytail: keep half-res on resize too — the upscaling happens via
        // the 100% width/height CSS on the canvas element set at mount.
        rendererRef.current.setSize(newWidth * 0.5, newHeight * 0.5, false);
        materialRef.current.uniforms.uResolution.value.set(newWidth * 0.5, newHeight * 0.5);
      }, 150);
    };
